...
if __name__ == "__main__":
    ...
    with open(test_file_dir,'r') as f:
        ...
        json_object = list(filter(lambda elem: 'weakener_prediction' in elem, json_object))
        print(len(json_object))
        ...
        for data in tqdm(json_object):
            ...
            naive_score['rougeL'] += data['weakener_prediction']['naive'][idx]['rougeL']
            naive_score['bertscore'] += data['weakener_prediction']['naive'][idx]['bertscore']
            naive_score['sacrebleu'] += data['weakener_prediction']['naive'][idx]['sacrebleu']
            naive_score['meteor'] += data['weakener_prediction']['naive'][idx]['meteor']
            naive_score['cossim'] += data['weakener_prediction']['naive'][idx]['cossim']
            
            
            
            # choose among cot random
            idx2 = random.choice(range(0,5))
            cot_random_score['rougeL'] += data['weakener_prediction']['cot'][idx2]['rougeL']
            cot_random_score['bertscore'] += data['weakener_prediction']['cot'][idx2]['bertscore']
            cot_random_score['sacrebleu'] += data['weakener_prediction']['cot'][idx2]['sacrebleu']
            cot_random_score['meteor'] += data['weakener_prediction']['cot'][idx2]['meteor']
            cot_random_score['cossim'] += data['weakener_prediction']['cot'][idx2]['cossim']
            
            # choose among cot
            max_sim = 0.0
            final_d = {}
            for idx,d in enumerate(data['weakener_prediction']['cot']):
                exp = d['explanation']
                exp = "[CLS] "+exp
